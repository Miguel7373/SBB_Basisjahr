...
public class Levin extends GameCharacters{
    public Levin(String name, double hp, int loadCapacity, Weapons weapons, int initiative, Armor armor, double maxHP, int coins, double damage, int defens) {
    ...
    public double levinDamage(Weapons weapons) {
        if (getDamage()>= 10) {
            ...
            if (weapons instanceof Sword || weapons instanceof Club) {
                ...
                double attackwithRangedWeapons = weapons.getAttackValue() * Main.getRandom(0.9, 1.1) + 50;
                return attackwithRangedWeapons;
            } else {
                double attackWithHand = getDamage() + 10;
